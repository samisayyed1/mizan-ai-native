...
use mizan_device_sync::engine as shared_sync_engine;
use mizan_device_sync::{
...
use mizan_storage_sqlite::sync::SyncTableRowCount;
...
fn is_pairing_already_confirmed_error(err: &mizan_device_sync::DeviceSyncError) -> bool {
    match err {
        mizan_device_sync::DeviceSyncError::Api {
            status,
            code,
            message,
            ..
...
fn is_pairing_already_approved_error(err: &mizan_device_sync::DeviceSyncError) -> bool {
    match err {
        mizan_device_sync::DeviceSyncError::Api {
            status,
            code,
...
impl shared_sync_engine::ReadyReconcileStore for TauriReadyReconcileRunner {
    async fn get_sync_state(&self) -> Result<mizan_device_sync::SyncState, String> {
        let token = self
            .context
            .connect_service()
...
fn sha256_checksum(bytes: &[u8]) -> String {
    mizan_device_sync::crypto::sha256_checksum(bytes)
}
...
fn encrypt_sync_payload(
    ...
    let dek = mizan_device_sync::crypto::derive_dek(root_key, key_version)
        .map_err(|e| format!("Failed to derive event DEK: {}", e))?;
    mizan_device_sync::crypto::encrypt(&dek, plaintext_payload)
...
fn decrypt_sync_payload(
    ...
    let dek = mizan_device_sync::crypto::derive_dek(root_key, key_version)
        .map_err(|e| format!("Failed to derive event DEK: {}", e))?;
    mizan_device_sync::crypto::decrypt(&dek, encrypted_payload)
...
pub async fn device_sync_bootstrap_snapshot_if_needed(
    ...
    let should_start_engine = match context.connect_service().get_valid_access_token().await {
        Ok(token) => context
            ...
            .get_sync_state(&token)
            .await
            .map(|sync_state| sync_state.state == mizan_device_sync::SyncState::Ready)
            .unwrap_or_else(|err| {
                log::warn!(
...
pub async fn complete_pairing_with_transfer(
    ...
    client
        .complete_pairing(
            ...
            &device_id,
            &pairing_id,
            mizan_device_sync::CompletePairingRequest {
                encrypted_key_bundle,
                sas_proof,
                signature,
            },
...
pub async fn confirm_pairing_with_bootstrap(
    ...
    match client
        .confirm_pairing(
            ...
            &device_id,
            &pairing_id,
            mizan_device_sync::ConfirmPairingRequest { proof },
        )
        .await
    {
        Ok(_) => {}
...
pub async fn begin_pairing_confirm(
    ...
    match client
        .confirm_pairing(
            ...
            &device_id,
            &pairing_id,
            mizan_device_sync::ConfirmPairingRequest { proof: Some(proof) },
        )
        .await
    {
        Ok(_) => {}
    ...
    if let Some(min_created_at) = min_snapshot_created_at.as_deref() {
        if let Ok(parsed_min) = mizan_device_sync::parse_sync_datetime_to_utc(min_created_at) {
            ...
            if parsed_min > max_allowed {
                ...
                match mizan_device_sync::normalize_sync_datetime(min_created_at) {
                    Ok(normalized) => {
                        set_min_snapshot_created_at_in_store(&device_id, &normalized);
...
mod tests {
    ...
    fn pairing_already_approved_error_is_detected() {
        let err = mizan_device_sync::DeviceSyncError::api_structured(
            409,
