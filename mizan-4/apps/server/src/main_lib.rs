...
use mizan_core::addons::{AddonService, AddonServiceTrait};
use mizan_core::{
...
pub struct AppState {
    ...
    pub performance_service:
        Arc<dyn mizan_core::portfolio::performance::PerformanceServiceTrait + Send + Sync>,
...
pub async fn build_state(config: &Config) -> anyhow::Result<Arc<AppState>> {
    ...
    let (sync_outbox_wake_sender, _sync_outbox_wake_receiver) = tokio::sync::mpsc::channel(128);
    ...
    let custom_provider_repository = Arc::new(
        mizan_storage_sqlite::custom_provider::CustomProviderSqliteRepository::new(
            pool.clone(),
            writer.clone(),
        ),
    );
    ...
    let performance_service = Arc::new(
        mizan_core::portfolio::performance::PerformanceService::new_with_timezone(
    ...
    // Create the AI environment and chat service using the new mizan-ai crate
    ...
    let device_display_name = "Mizan Server".to_string();
