...
pub struct QueueWorkerDeps {
    ...
    pub health_service: Arc<dyn mizan_core::health::HealthServiceTrait + Send + Sync>,
    ...
    pub snapshot_service:
        Arc<dyn mizan_core::portfolio::snapshot::SnapshotServiceTrait + Send + Sync>,
    pub quote_service: Arc<dyn mizan_core::quotes::QuoteServiceTrait + Send + Sync>,
    pub valuation_service:
        Arc<dyn mizan_core::portfolio::valuation::ValuationServiceTrait + Send + Sync>,
    pub account_service: Arc<mizan_core::accounts::AccountService>,
    pub goal_service: Arc<dyn GoalServiceTrait + Send + Sync>,
    pub fx_service: Arc<dyn mizan_core::fx::FxServiceTrait + Send + Sync>,
...
async fn run_portfolio_job(
    ...
    let snapshot_mode = config
        .since_date
        .map(mizan_core::portfolio::snapshot::SnapshotRecalcMode::SinceDate)
        .unwrap_or_else(|| config.snapshot_mode.clone());
    let valuation_mode = config
        .since_date
        .map(mizan_core::portfolio::valuation::ValuationRecalcMode::SinceDate)
    ...
    if config.market_sync_mode.requires_sync() {
        ...
        let sync_result = match config.market_sync_mode.to_sync_mode() {
            ...
            None => {
                ...
                Ok(mizan_core::quotes::SyncResult::default())
            }
        };
...
impl mizan_connect::SyncProgressReporter for EventBusProgressReporter {
    fn report_progress(&self, payload: mizan_connect::SyncProgressPayload) {
    ...
    fn report_sync_complete(&self, result: &mizan_connect::SyncResult) {
...
) -> Result<mizan_connect::SyncResult, String> {
    use mizan_connect::{ConnectApiClient, SyncConfig, SyncOrchestrator};
