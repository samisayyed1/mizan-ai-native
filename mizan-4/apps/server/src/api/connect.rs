//! Mizan Connect API endpoints for broker synchronization.
//!
//! This module provides REST endpoints for syncing broker accounts and activities
//! from the Mizan Connect cloud service.
...
fn connect_auth_url() -> Option<String> {
    std::env::var("CONNECT_AUTH_URL")
        ...
        .or_else(|| Some("https://auth.mizan.app".to_string()))
}
...
/// Core Gold sync logic. Mizan Connect owns Plaid tokens and normalization;
/// this app triggers the server-side sync and receives the computed snapshot
/// through the encrypted local sync boundary.
...
/// Resolve the current user's entitlements. Signed-out / errors fall back
/// to Silver defaults, so web mode degrades the same way the desktop does.
...
async fn create_plaid_link_token(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Json<mizan_connect::PlaidLinkTokenResponse>> {
    let client = create_connect_client(&state).await?;
    let response = client
...
#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct ExchangePlaidPublicTokenRequest {
    public_token: String,
}

async fn exchange_plaid_public_token(
    State(state): State<Arc<AppState>>,
    Json(req): Json<ExchangePlaidPublicTokenRequest>,
) -> ApiResult<Json<mizan_connect::PlaidExchangePublicTokenResponse>> {
    let client = create_connect_client(&state).await?;
    let response = client
        .exchange_plaid_public_token(&req.public_token)
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
    Ok(Json(response))
}
...
async fn list_broker_connections(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Json<Vec<mizan_connect::broker::BrokerConnection>>> {
    ensure_connect_sync_enabled()?;
...
async fn delete_broker_connection(
    State(state): State<Arc<AppState>>,
...
async fn list_broker_accounts(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Json<Vec<mizan_connect::broker::BrokerAccount>>> {
    ensure_connect_sync_enabled()?;
...
async fn get_synced_accounts(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Json<Vec<mizan_core::accounts::Account>>> {
    if !crate::features::connect_sync_enabled() {
        return Ok(Json(vec![]));
    }
...
async fn get_platforms(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Json<Vec<mizan_connect::Platform>>> {
    if !crate::features::connect_sync_enabled() {
        return Ok(Json(vec![]));
    }
...
async fn get_broker_sync_states(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Json<Vec<mizan_connect::BrokerSyncState>>> {
    if !crate::features::connect_sync_enabled() {
        return Ok(Json(vec![]));
    }
...
) -> ApiResult<Json<Vec<mizan_connect::ImportRun>>> {
    if !crate::features::connect_sync_enabled() {
        return Ok(Json(vec![]));
    }
...
) -> ApiResult<Json<mizan_core::activities::BrokerSyncProfileData>> {
    Ok(Json(
        state
            .activity_service
...
async fn save_broker_sync_profile_rules(
    State(state): State<Arc<AppState>>,
    Json(request): Json<mizan_core::activities::SaveBrokerSyncProfileRulesRequest>,
) -> ApiResult<Json<mizan_core::activities::BrokerSyncProfileData>> {
    Ok(Json(
        state
            .activity_service
...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/connect/plaid/exchange-public-token",
            post(exchange_plaid_public_token),
        )
