...
export const COMMANDS: CommandMap = {
  ...
  create_plaid_link_token: { method: "POST", path: "/connect/plaid/link-token" },
  exchange_plaid_public_token: { method: "POST", path: "/connect/plaid/exchange-public-token" },
...
export const invoke = async <T>(command: string, payload?: Record<string, unknown>): Promise<T> => {
  ...
  switch (command) {
    ...
    case "reinitialize_device_sync":
      break;
    case "exchange_plaid_public_token": {
      const { publicToken } = payload as { publicToken: string };
      body = JSON.stringify({ publicToken });
      break;
    }
    case "delete_broker_connection": {
      const { connectionId } = payload as { connectionId: string };
      url = url.replace(":connection_id", encodeURIComponent(connectionId));
      break;
    }
