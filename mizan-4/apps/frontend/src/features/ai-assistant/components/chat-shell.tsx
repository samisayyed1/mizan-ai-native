...
import { Badge, EmptyPlaceholder } from "@mizan/ui";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
function Header({
  ...
  initialIntent?: ChatShellProps["initialIntent"];
}) {
  return (
    <header className="pt-safe flex shrink-0 items-center gap-2 border-b px-4">
      ...
      <ProviderPicker />
      {initialIntent?.intent === "add-asset" && (
        <Badge variant="secondary" className="hidden cursor-default md:inline-flex">
          Asset ingest
        </Badge>
      )}
 ...
 */
function IntentPromptCard({ initialIntent }: { initialIntent?: ChatShellProps["initialIntent"] }) {
  if (!initialIntent) return null;

  return (
    <div className="border-primary/20 bg-primary/5 mx-auto mt-4 w-full max-w-[44rem] rounded-xl border p-3">
      <div className="flex items-start gap-3">
        <div className="bg-primary/10 text-primary flex size-9 shrink-0 items-center justify-center rounded-lg">
          <Icons.Sparkles className="size-4" />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium">Add through Assistant</p>
          <p className="text-muted-foreground mt-1 text-sm leading-5">{initialIntent.prompt}</p>
          <ThreadPrimitive.Suggestion prompt={initialIntent.prompt} asChild>
...
function ChatShellInner({ className, initialIntent }: ChatShellProps) {
        ...
        <div className={cn("bg-background flex h-full min-h-0 w-full", className)}>
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            <main className="flex min-h-0 flex-1 flex-col overflow-hidden">
              <IntentPromptCard initialIntent={initialIntent} />
 ...
 */
export function ChatShell({ className, initialIntent }: ChatShellProps) {
  return (
    <ChatModelProvider>
      <ChatShellInner className={className} initialIntent={initialIntent} />
    </ChatModelProvider>
  );
}
