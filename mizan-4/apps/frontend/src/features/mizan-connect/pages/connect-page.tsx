...
import { ConnectEmptyState } from "@/features/mizan-connect/components/connect-empty-state";
import {
...
export default function ConnectPage() {
  ...
  // header CTA so all entry points go through the Plaid Link flow.
  ...
  // Silver users (no plaidSync capability) still see the empty state with
  // the upgrade-to-Gold CTA; same for not-signed-in users. `showBrokerSync`
  // is the canonical Gold gate.
  if (!isEnabled || !isConnected || !showBrokerSync) {
    return (
      <Page>
        <PageHeader heading="Sync & Connections" />
        <PageContent>
