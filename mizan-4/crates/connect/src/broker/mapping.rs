...
use mizan_core::activities::{self, AssetResolutionInput, NewActivity};
use mizan_core::assets::parse_symbol_with_exchange_suffix;
use mizan_core::fx::currency::{get_normalization_rule, normalize_amount, resolve_currency};
...
pub fn map_broker_activity(
    ...
    let status = if needs_review_flag {
        mizan_core::activities::ActivityStatus::Draft
    } else {
        mizan_core::activities::ActivityStatus::Posted
    };
...
mod tests {
    ...
    fn test_map_broker_activity_uses_provider_and_external_reference_fallbacks() {
        let activity = AccountUniversalActivity {
            ...
            activity_type: Some("BUY".to_string()),
            provider_type: Some("PLAID".to_string()),
        ...
        assert_eq!(mapped.source_system.as_deref(), Some("PLAID"));
        ...
        assert_eq!(metadata["provider_type"], "PLAID");
