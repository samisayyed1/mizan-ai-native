...
import { type AddonContext } from '@mizan/addon-sdk';
...
mod service_tests {
    ...
    fn test_ensure_addons_directory_service() {
        ...
        let temp_dir = env::temp_dir().join("mizan_test_addons");
    ...
    fn test_get_addon_path_service() {
        ...
        let temp_dir = env::temp_dir().join("mizan_test_addon_path");
        let app_data_path = temp_dir.to_str().unwrap();
    ...
    fn test_addon_service_load_manifest() {
        ...
        let temp_dir = env::temp_dir().join("mizan_test_manifest_service");
        let app_data_path = temp_dir.to_str().unwrap();
    ...
    async fn test_install_addon_zip_rejects_unsafe_paths_without_writing_files() {
        let temp_dir = env::temp_dir().join("mizan_test_addon_zip_traversal");
        if temp_dir.exists() {
            std::fs::remove_dir_all(&temp_dir).ok();
        }
