...
use mizan_market_data::{exchanges_for_currency, mic_to_exchange_name};
    ...
    R: ActivityRepositoryTrait + 'static,
{
    ...
    fn build_extra_providers(
        custom_provider_repo: &Option<Arc<dyn crate::custom_provider::CustomProviderRepository>>,
        secret_store: &Arc<dyn SecretStore>,
    ) -> Vec<Arc<dyn mizan_market_data::MarketDataProvider>> {
        let mut extra: Vec<Arc<dyn mizan_market_data::MarketDataProvider>> = Vec::new();
        ...
        for attempt_symbol in symbol_resolution_candidates(clean_symbol) {
            ...
            let bond_metadata = if instrument_type == Some(&InstrumentType::Bond) {
                ...
                if isin.starts_with("US912") {
                    ...
                    mizan_market_data::provider::us_treasury_calc::UsTreasuryCalcProvider::fetch_bond_details(&http, &isin).await
                        .map(|details| {
