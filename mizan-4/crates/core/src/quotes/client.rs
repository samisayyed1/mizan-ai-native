...
pub enum MarketDataClientError {
    ...
    #[error("{0}")]
    MarketData(#[from] mizan_market_data::errors::MarketDataError),
...
impl MarketDataClient {
    ...
    pub async fn new_with_extra(
        secret_store: Arc<dyn SecretStore>,
        enabled_providers: Vec<ProviderConfig>,
        extra_providers: Vec<Arc<dyn mizan_market_data::MarketDataProvider>>,
    ) -> Result<Self> {
        use std::collections::HashMap;

        let mut providers: Vec<Arc<dyn mizan_market_data::MarketDataProvider>> = Vec::new();
    ...
    async fn create_provider(
        provider_id: &str,
        secret_store: &Arc<dyn SecretStore>,
    ) -> Result<Option<Arc<dyn mizan_market_data::MarketDataProvider>>> {
        match provider_id {
            DATA_SOURCE_YAHOO => {
    ...
    fn build_quote_context(&self, asset: &Asset) -> Result<QuoteContext> {
        ...
        let overrides = asset
            .provider_overrides()
            .and_then(|json| mizan_market_data::ProviderOverrides::from_json(json).ok());
...
mod tests {
    ...
    fn test_build_quote_context_equity() {
        ...
        match instrument.unwrap() {
            mizan_market_data::InstrumentId::Equity { ticker, mic } => {
    ...
    fn test_build_quote_context_crypto() {
        ...
        match instrument.unwrap() {
            mizan_market_data::InstrumentId::Crypto { base, quote } => {
    ...
    fn test_build_quote_context_fx() {
        ...
        match instrument.unwrap() {
            mizan_market_data::InstrumentId::Fx { base, quote } => {
