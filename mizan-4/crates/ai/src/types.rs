...
impl Default for ChatThreadConfig {
    fn default() -> Self {
        Self {
            ...
            model_id: String::new(),
            prompt_template_id: "mizan-assistant-v1".to_string(),
...
mod tests {
    ...
    fn test_thread_config_creation() {
        let config = ChatThreadConfig::new("openai", "gpt-4o", "mizan-assistant-v1", "1.0.0");
        assert_eq!(config.provider_id, "openai");
        assert_eq!(config.model_id, "gpt-4o");
        assert_eq!(config.prompt_template_id, "mizan-assistant-v1");
