...
use mizan_core::errors::{Result, ValidationError};
use mizan_core::secrets::SecretStore;
use mizan_core::settings::SettingsRepositoryTrait;
...
impl AiProviderServiceTrait for AiProviderService {
    ...
    async fn update_provider_settings(&self, request: UpdateProviderSettingsRequest) -> Result<()> {
        ...
        if !self.catalog.providers.contains_key(&request.provider_id) {
            return Err(mizan_core::errors::Error::Validation(
            ...
            match tuning_update {
                Some(overrides) => {
                    overrides.validate().map_err(|msg| {
                        mizan_core::errors::Error::Validation(ValidationError::InvalidInput(
