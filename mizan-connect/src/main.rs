...
async fn main() -> anyhow::Result<()> {
    ...
    let plaid = match config.plaid.as_ref() {
        Some(plaid_config) => {
            let token_cipher = SecretCipher::from_bytes(&plaid_config.token_encryption_key)
                .context("MIZAN_PLAID_TOKEN_ENCRYPTION_KEY length validation")?;
            token_cipher
                .self_test()
        ...
        None => {
            tracing::warn!("Plaid is not configured — Gold live sync endpoints return 503");
            None
        }
    };
    ...
    let billing = config.billing.as_ref().map(|bc| {
        ...
        let prices = StripePrices {
            silver_monthly: bc.stripe_prices.silver_monthly.clone(),
            silver_yearly: bc.stripe_prices.silver_yearly.clone(),
            gold_monthly: bc.stripe_prices.gold_monthly.clone(),
            gold_yearly: bc.stripe_prices.gold_yearly.clone(),
        };
        BillingContext::new(
            stripe,
            prices,
    ...
    let state = AppState::new(config.clone(), pool.clone(), jwks, plaid, billing);
