...
| A7 | ✅ Done (2026-06-04 as PR-A8 per v3 Goal §V) | Heatmap tile-tap → asset detail. Extracted `holdingDetailRouteFromNode()` pure helper, 9 vitest tests covering helper invariants (URL-encode, null guards, ignores extras) + render branches (loading / empty / cash-excluded / alt-asset-excluded / mode toggle). Click-through Playwright assertion lands with PR-E2E. |
...
| Sankey allocation flow | ✅ Done (2026-06-04 as PR-NW2 per Goal v3 §V Phase 7) | `components/charts/sankey.tsx` wraps recharts `<Sankey>` with the chart-* palette theming used by donut/bar/heatmap/sparkline; `asset-allocation-flow.ts` rollup computes Net Worth → asset-class flows excluding vehicles + liabilities (per Feroz #14); `net-worth-allocation-section.tsx` integrates onto the Net Worth page between history chart and Composition grid with height scaled by class count, empty-state fallback, and stale-load guard. Income → spending → savings cash-flow Sankey deferred to PR-NW2.b once activities are threaded through the frontend. **10 unit tests** including §23-flavored 5-panel fixture (Real Estate $1.65M + Bonds $688K + Equities $500K + PE $400K + Cash $300K = $3.538M Net Worth, vehicles + liabilities excluded). Frontend vitest 1180/1180; lint clean; tsc clean. |
