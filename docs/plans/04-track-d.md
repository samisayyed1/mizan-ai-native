...
| D2.b | ✅ Done (2026-06-04 as PR-D2.b per Goal v3 §V Phase 6 — `POST /v1/news/feed` handler over the NewsAPI provider + personalization stack; PR-D3 grafts pgvector similarity on top, PR-D4 adds the per-user materialized view + cursor pagination) | News feed handler. New `mizan-connect/src/news/handlers.rs` with `feed_handler` accepting `?tab=relevant\|global&query=...&language=...&page_size=N` (defaults: global / "finance OR markets OR stocks OR economy" / 30 / NewsAPI ISO-639-1 en) + POST body `FeedBody { holdingSymbols, holdingCategories, memoryKeywords }` (camelCase serde) → `FeedResponse { tab, provider: "newsapi", articles: Vec<RankedArticle> }`. Reads `MIZAN_NEWSAPI_KEY` per-request (PR-D2.c promotes to state-injected once additional providers ship). Composes `fetch_newsapi` → `rank_articles` (NewsTab::Global passes `RankingInput::default()` so global tab returns time-ranked unbiased feed). `map_newsapi_error` translates MissingApiKey → 503 service_unavailable (graceful fallback to desktop TradingView path), Transport/HttpError/BadBody → 502 bad_gateway with provider context preserved. Wired into `news::router()` at `POST /v1/news/feed`. **10 new unit tests**: default-tab Global, default-query non-empty, default-page-size 30, body defaults to empty lists, body → RankingInput conversion preserves all fields, partial body deserialisation (`holdingSymbols` only), 4 error-mapping tests (each NewsApiError variant). Cargo check / clippy / fmt clean; 45/45 news tests pass (was 35; +10). |
