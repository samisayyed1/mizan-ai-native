...
| C5.d.7 | ✅ Done (2026-06-04) | Tax-window scheduler hydration — **completes 6 of 7 Phase 4 rule families**. New pure-math `compute_tax_optimization_windows(today) → Vec<TaxOptimizationWindow>` reads a static `TAX_DEADLINE_CALENDAR` (5 entries: CPF SA top-up Dec 31, IRA April 15 of N+1, capital-gains harvest Dec 31, 401(k) deferral Dec 31, NPS Tier-I March 31). `next_occurrence(today, entry)` resolves the next yearly recurrence, honouring `next_year_cutoff:true` for IRA semantics (cutoff for tax year N is April 15 of N+1). Sort by `days_remaining` ascending so the bell panel renders the most-urgent first. `potential_savings_base` left `None` until per-user marginal-rate input ships (engine renders the line conditionally). Wired into `InsightsInput` section 11. **10 unit tests** including the Jan→Dec rollover, the IRA N+1 invariant, the §23-flavored SG+US+IN jurisdiction coverage pin, sort-ordering regression, savings-None invariant. mizan-app 103/103 (was 93; +10); clippy clean. PR-C5.d.6 (cash-drag-opportunity) remains the only hydration slice not yet wired — needs a yield-discovery worker that ships with Track B cash providers. |
| C5.d.5 | ✅ Done (2026-06-04) | **Sharia-status scheduler hydration — completes 5 of 7 Phase 4 rule families.** New `mizan_storage_sqlite::holdings_metadata::HoldingsMetadataRepository` (read-only) + `detect_flips` pure-math helper grouping `holdings_metadata` rows by `(account_id, holding_symbol)`, finding the latest verdict ≤ today + the most-recent prior verdict that differs, emitting `ShariaStatusFlip[]`. ServiceContext gets `holdings_metadata_repository`. `scheduler.rs::hydrate_sharia_status_changes(context, today)` converts flips → `ShariaStatusChange` candidates with composite `holding_id = "account_id:holding_symbol"` for the bell-panel dedupe-key surface. Diesel `holdings_metadata` table macro added forward-only (12 columns matching the PR-E1 migration). **9 unit tests** on `detect_flips`: empty input, first-time-screening (no flip), same-verdict no-flip, compliant→mixed flip, future-rows filtered, multiple-holdings independent, back-and-forth latest-transition, unsorted-input handled, null-verdicts skipped. Engine's `eval_sharia_status_change` re-validates `prior != new` (defence-in-depth) and emits a Warning notification. storage-sqlite 183/183 (was 174; +9); mizan-app 93/93; clippy clean. PR-C5.d.6+ will hydrate cash-drag-opportunity + tax-window. |
