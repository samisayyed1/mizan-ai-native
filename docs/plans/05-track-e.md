...
| E4 | ✅ Done (2026-06-04) | AAOIFI screening worker on Mizan Connect — `mizan_connect::sharia` module + `aaoifi_rules::screen()` pure function + `POST /v1/sharia/screen` + `GET /v1/sharia/status/:symbol` per ADR 0012. Decimal-everywhere math. 12 golden tests covering every verdict path (Unrated/NonCompliant/Mixed/Compliant), edge cases (debt at-cap vs over-cap, partial ratios, multi-failure rationale). Persistence (E4.b) + GICS lookup (E4.c) + SEC EDGAR fetcher (E4.d) deferred to follow-ups — surface ships now so the desktop badge + the `find_sharia_status` agent tool (PR-C4.c) can wire against a stable URL. |
